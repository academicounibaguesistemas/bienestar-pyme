 ...
 * encuestas guardadas en localStorage), los resultados por dimension
 * (Sprint 7, ver DimensionesResumen en indicadores.js), el desglose real
 * de indicadores por area, cargo y antiguedad con su ranking ejecutivo
 * (Sprint 4A y Sprint 4B, ver indicadores.js), el resumen de hallazgos
 * principales generado automaticamente (HallazgosPrincipales), las
...
const Reportes = {
    render() {
      ...
      const info = Indicadores.calcularIndicadoresActuales();
          const resumenDimensiones = DimensionesResumen.calcular();
                                                                                                                                                                                                  ...
                                                                                                                                                                                                  <p class="text-muted mt-md">
                                                                                                                                                                                                          ${info.hayDatosReales
                                                                                                                                                                                                                      ...
                                                                                                                                                                                                                      : "Aún no se han registrado encuestas · mostrando datos de demostración."}
                                                                                                                                                                                                                                  ...
                                                                                                                                                                                                                                  <h3 class="mt-md">Resultados por dimensión</h3>
                                                                                                                                                                                                                                        ${this.resultadosPorDimensionHTML(resumenDimensiones)}
    ...
    /**
         * Sprint 7: tabla "Resultados por dimensión", con una fila por cada una
     * de las 6 dimensiones de la encuesta (DimensionesResumen.calcular, ver
     * indicadores.js): su puntaje 0-100 y su estado tipo semaforo
     * (Excelente/Aceptable/Crítico, reutilizando semaforoHTML de utils.js).
     * Debajo de la tabla se listan las recomendaciones automaticas fijas
     * por dimension (RECOMENDACIONES_DIMENSION, ver data.js). Mientras no
     * existan encuestas reales, se muestran los puntajes de demostracion
     * (DIMENSIONES_DEMO), igual que el resto de KPIs generales.
     */
    resultadosPorDimensionHTML(resumen) {
          if (!resumen.lista.length) {
                  return `<p class="text-muted">Aún no hay datos suficientes para calcular los resultados por dimensión.</p>`;
          }

      return `
            <table id="tDimensiones">
                    <thead><tr><th>Dimensión</th><th>Puntaje</th><th>Estado</th></tr></thead>
                            <tbody>
                                      ${resumen.lista.map(d => `
                                                  <tr>
                                                                <td>${d.nombre}</td>
                                                                              <td>${d.puntaje}</td>
                                                                                            <td>${semaforoHTML(d.puntaje)}</td>
                                                                                                        </tr>
                                                                                                                  `).join("")}
                                                                                                                          </tbody>
                                                                                                                                </table>
                                                                                                                                      <div class="mt-md">
                                                                                                                                              ${resumen.lista.map(d => `<p><strong>${d.nombre}:</strong> ${RECOMENDACIONES_DIMENSION[d.id]}</p>`).join("")}
                                                                                                                                                    </div>
                                                                                                                                                        `;
    },
