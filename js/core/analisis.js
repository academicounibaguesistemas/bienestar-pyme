 ...
 *
 * Sprint 7: agrega a los hallazgos del sistema una frase sobre las 6
 * dimensiones de la encuesta (ver DimensionesResumen en indicadores.js),
 * mencionando cual es la de mejor desempeño y cual la de mayor riesgo.
 */
...
const HallazgosSistema = {
    ...
    /**
         * Sprint 7: hallazgo adicional sobre las 6 dimensiones de la encuesta,
     * reutilizando DimensionesResumen.calcular (ver indicadores.js).
     * Menciona directamente cual dimension tiene el mejor desempeño y
     * cual presenta el mayor riesgo. Igual que hallazgoAreaMenorDesempeno,
     * solo se muestra cuando ya hay encuestas reales registradas (nunca se
     * simula cual dimension es mejor o peor con datos de demostracion).
     */
    hallazgoDimensiones() {
          const resumen = DimensionesResumen.calcular();
          if (!resumen.hayDatosReales) return null;
          return {
                  texto: "La dimensión con mejor desempeño es \"" + resumen.mejor.nombre + "\" (" + resumen.mejor.puntaje + " pts) y la que presenta mayor riesgo es \"" + resumen.peor.nombre + "\" (" + resumen.peor.puntaje + " pts).",
                  recomendaciones: [RECOMENDACIONES_DIMENSION[resumen.peor.id]],
          };
    },
    ...
    generar() {
      ...
      const dimensiones = this.hallazgoDimensiones();
          if (dimensiones) hallazgos.push(dimensiones);
